...
- Фоновый listener не отправляет Telegram-сообщения, не удаляет их, не ставит реакции и не меняет настройки. Исключение — ручная отправка текста, фото или файла из приватного inbox, Избранного и явно разрешённого Library-бота.
...
«Библиотека» управляется из редкого drawer в web UI и хранит выбранные Telegram-диалоги,
порядок, local mute, разрешение на запись bot и будущий `digest_excluded` в SQLite.
`data/library_chats.json` теперь только одноразовый совместимый seed: startup не сканирует
Telegram-диалоги. Первым всегда идёт ☆ Избранное. Обычные источники read-only, история
загружается явно по 50 сообщений; доступны отдельные закрепы и поиск только внутри
открытого source. Входящее сообщение выбранного source создаёт одну временную Inbox-
проекцию с local unread, но source остаётся в Библиотеке. Local mute подавляет native
banner, не меняя Telegram notification settings.
...
Избранное использует серверное планирование по схеме AyuGram: 12 секунд для текста
