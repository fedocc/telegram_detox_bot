...
class LibrarySource(Base):
    ...
    manual_write_enabled: Mapped[bool] = mapped_column(Boolean, default=False, index=True)
    manual_open_date: Mapped[date | None] = mapped_column(Date, nullable=True)
    manual_access_until: Mapped[float] = mapped_column(Float, default=0)
    last_seen_message_id: Mapped[int] = mapped_column(Integer, default=0)
